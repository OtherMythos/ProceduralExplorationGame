::PlacedItems <- array(PlacedItemId.MAX, null);

::PlacedItems[PlacedItemId.TREE] = PlacedItem("tree.voxMesh", 0.6);

::PlacedItems[PlacedItemId.CHERRY_BLOSSOM_TREE] = PlacedItem("treeCherryBlossom.voxMesh", 0.6);
::PlacedItems[PlacedItemId.CACTUS] = PlacedItem("cactus1.voxMesh", 0.3);
::PlacedItems[PlacedItemId.TREE_APPLE] = PlacedItem("treeApple.voxMesh", 0.6);
::PlacedItems[PlacedItemId.PALM_TREE] = PlacedItem("palmTree.voxMesh", 0.3);
::PlacedItems[PlacedItemId.PALM_TREE_COCONUTS] = PlacedItem("coconutPalmTree.voxMesh", 0.3);
::PlacedItems[PlacedItemId.FLOWER_PURPLE] = PlacedItem("flower.flowerPurple.voxMesh", 0.3);
::PlacedItems[PlacedItemId.FLOWER_RED] = PlacedItem("flower.flowerRed.voxMesh", 0.3);
::PlacedItems[PlacedItemId.FLOWER_WHITE] = PlacedItem("flower.flowerWhite.voxMesh", 0.3);
::PlacedItems[PlacedItemId.BERRY_BUSH] = PlacedItem("berryBush.voxMesh", 0.2);
::PlacedItems[PlacedItemId.BERRY_BUSH_BERRIES] = PlacedItem("berryBushBerries.voxMesh", 0.2);

::PlacedItems[PlacedItemId.SWAMP_TREE_ONE] = PlacedItem("swampPlants.tree1.voxMesh", 0.4);
::PlacedItems[PlacedItemId.SWAMP_TREE_TWO] = PlacedItem("swampPlants.tree2.voxMesh", 0.4);
::PlacedItems[PlacedItemId.SWAMP_BULLRUSH_1] = PlacedItem("swampPlants.bullrush1.voxMesh", 0.2);
::PlacedItems[PlacedItemId.SWAMP_BULLRUSH_2] = PlacedItem("swampPlants.bullrush2.voxMesh", 0.2);
::PlacedItems[PlacedItemId.SWAMP_GRASS_1] = PlacedItem("swampPlants.waterBush1.voxMesh", 0.4);
::PlacedItems[PlacedItemId.SWAMP_GRASS_2] = PlacedItem("swampPlants.waterBush2.voxMesh", 0.2);
::PlacedItems[PlacedItemId.SWAMP_LILYPAD] = PlacedItem("swampPlants.lilypad.voxMesh", 0.2, Vec3(0, -0.8, 0));