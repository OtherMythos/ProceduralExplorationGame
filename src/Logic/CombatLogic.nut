...
::CombatLogic <- class{
    ...
    function notifyAllOpponentsDied_(){
        ...
        mData_.resetSpoils();
        mData_.setSpoilForIdx(::FoundObject(::Item(ItemId.SIMPLE_SWORD), FoundObjectType.ITEM), 0);
        mData_.setSpoilForIdx(::FoundObject(::Item(ItemId.SIMPLE_SHIELD), FoundObjectType.ITEM), 1);
    ...
    function scrapAllSpoils(){
        ...
        for(local i = 0; i < data.len(); i++){
            ...
            ::Base.mInventory.addMoney(item.obj.getScrapVal());
            data[i] = null;
        }
    ...
    function scrapSpoil(slotIdx){
        ...
        ::Base.mInventory.addMoney(spoilItem.obj.getScrapVal());
