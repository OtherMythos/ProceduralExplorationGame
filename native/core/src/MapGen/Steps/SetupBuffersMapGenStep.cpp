...
namespace ProceduralExplorationGameCore{
    ...
    void SetupBuffersMapGenStep::processStep(const ExplorationMapInputData* input, ExplorationMapData* mapData){
        ...
        AV::uint8* startPtr = static_cast<AV::uint8*>(start);

        mapData->voxelBuffer = static_cast<void*>(startPtr);
        mapData->secondaryVoxelBuffer = static_cast<void*>(startPtr + VOX_BUF_SIZE);
        mapData->blueNoiseBuffer = static_cast<void*>(startPtr + VOX_BUF_SIZE + SECONDARY_VOX_BUF_SIZE);
