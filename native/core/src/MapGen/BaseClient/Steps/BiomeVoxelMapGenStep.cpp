...
namespace ProceduralExplorationGameCore{
    ...
    bool BiomeVoxelMapGenStep::processStep(const ExplorationMapInputData* input, ExplorationMapData* mapData, ExplorationMapGenWorkspace* workspace){
    ...
    void BiomeVoxelMapGenJob::processJob(ExplorationMapData* mapData, WorldCoord xa, WorldCoord ya, WorldCoord xb, WorldCoord yb){
        ...
        for(int y = ya; y < yb; y++){
            for(int x = xa; x < xb; x++){
                ...
                if(fullTertiaryVox & DO_NOT_CHANGE_VOXEL){
                    continue;
                }
