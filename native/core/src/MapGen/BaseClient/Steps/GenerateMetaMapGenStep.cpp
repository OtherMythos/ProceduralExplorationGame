...
namespace ProceduralExplorationGameCore{
    ...
    bool GenerateMetaMapGenStep::processStep(const ExplorationMapInputData* input, ExplorationMapData* mapData, ExplorationMapGenWorkspace* workspace){
        ...
        AV::uint32 seedBase = static_cast<AV::uint32>((seed >> 16) & 0xFFFF);
        AV::uint32 moistureSeed = static_cast<AV::uint32>((seed >> 8) & 0xFF);
        AV::uint32 variationSeed = static_cast<AV::uint32>(seed & 0xFF);
        ...
        mapData->voidPtr("pathData", new std::vector<PathSegment>());
